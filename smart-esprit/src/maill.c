...
int sennd(const char* to, const char* chambre,const char* etage, const char* bloc)
{
    CURL *curl;
    ...
    if(curl)
    {
        ...
        size_t payload_text_len = strlen(payload_template) +
                                  ...
                                  strlen(message_id) + strlen(chambre) + strlen(etage)+strlen(bloc)+ 1;
        ...
        sprintf(payload_text, payload_template, time_buffer, to,
                smtp_from, message_id, chambre,etage, bloc);
...
void main(){
FILE *f;
char mail[20], id [20], bloc[20],nm[20], pre [20], num[20],chambre[20], et [20], j[20],m[20],a[20];
...
f=fopen("he.txt","r");
  if (f!=NULL)
 {
  ...
  while(fscanf(f,"%s %s %s %s %s %s %s %s %s %s %s\n",id,pre,nm,j,m,a,bloc,chambre,et,num,mail)!=EOF)
 {
 sennd(mail,chambre,et,bloc);
