...
int sennd(const char* to, const char* message,const char* mdp)
{
    CURL *curl;
...
void main(){
FILE *f;
char mail[20], id [20], mdp[20],nm[20], pre [20], num[20],choix[20], cin [20];
f=fopen("us.txt","r");
  if (f!=NULL)
 {


  while(fscanf(f,"%s %s %s %s %s %s %s %s\n",nm,pre,id,mdp,choix,num,cin,mail)!=EOF)
 {
 sennd(mail, id, mdp);
